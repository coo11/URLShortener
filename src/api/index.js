...
const ErrorCodes = {
  ...
  PRIVATE_USE: {code: 'PRIVATE_USE', message: 'Service is not available now.'}
};
...
async function shortenLink(event) {
  ...
  // Implement private use only.
  if (globalThis.HASHPWD) {
    if (slug.endsWith(`#${HASHPWD}`)) {
      slug = slug.replace(`#${HASHPWD}`,'');
    } else return respond(event, ErrorCodes.PRIVATE_USE, 400);
  }
  ...
  // Is short link exist?
  const isShortLinkInUse = await URLSHTNR.get(shortLink);
  ...
  // Everyting is ok.
  await URLSHTNR.put(shortLink, url);
